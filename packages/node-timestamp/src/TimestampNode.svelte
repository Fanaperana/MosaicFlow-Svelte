<!--
  TimestampNode - Entity Category
  
  Compact, live-updating date/time display.
-->
<script lang="ts">
  import { NodeResizer, Handle, Position, type NodeProps, type Node } from '@xyflow/svelte';
  import type { TimestampNodeData } from './types';
  ...
  let { id, data, selected }: NodeProps<TimestampNodeType> = $props();
