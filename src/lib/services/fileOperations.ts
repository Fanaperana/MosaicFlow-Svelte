...
// Export canvas as high-resolution PNG using headless Chrome in Rust
export async function exportAsPng(): Promise<boolean> {
  console.log('Starting high-res PNG export (via headless Chrome)...');
  ...
  try {
    ...
    // Step 3: Convert SVG to high-res PNG using headless Chrome in Rust
    const { invoke } = await import('@tauri-apps/api/core');
    
    // Use scale factor of 2 for high-DPI output (headless Chrome handles this well)
    const scale = 2.0;
    console.log(`Converting to PNG at ${scale}x scale via headless Chrome...`);
    console.log(`Output resolution: ${Math.round(svgResult.width * scale)}x${Math.round(svgResult.height * scale)}`);
    
    await invoke('svg_to_png_headless', {
