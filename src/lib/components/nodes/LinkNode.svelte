<script lang="ts">
  ...
  let { data, selected, id }: NodeProps<LinkNode> = $props();
  ...
  function handleUrlChange(e: Event) {
    ...
    url = target.value;
    workspace.updateNodeData(id, { url });
  }

  function handleDescChange(e: Event) {
    const target = e.target as HTMLInputElement;
    description = target.value;
    workspace.updateNodeData(id, { description });
  }
...
>
  <div class="node-header">
    ...
    {#if url}
      <button class="open-btn" onclick={openLink} title="Open link"><ExternalLink size={14} strokeWidth={1.5} /></button>
    {/if}
  </div>
