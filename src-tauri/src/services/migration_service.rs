...
use std::path::Path;
use crate::core::{self, MosaicResult, MosaicError, paths::{VaultPaths, CanvasPaths}};
use crate::models::{VaultMeta, VaultInfo, CanvasMeta, CanvasInfo, CanvasUIState};
...
impl MigrationService {
    ...
    pub fn migrate_vault(path: &Path) -> MosaicResult<VaultInfo> {
        ...
        Ok(VaultInfo::from_meta(&meta, path.to_string_lossy().to_string(), canvas_count))
    }
    ...
    pub fn migrate_canvas(path: &Path) -> MosaicResult<CanvasInfo> {
        ...
        let (canvas_id, vault_id, name, _created_at, _updated_at) = if old_canvas_json.exists() {
            ...
            let vault_id = Self::get_vault_id_from_canvas_path(path)
                .unwrap_or_else(core::generate_uuid);
