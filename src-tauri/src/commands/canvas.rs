...
use crate::services::{VaultService, CanvasService, HistoryService, StateService};
...
pub async fn create_canvas(
    ...
    // Update state
    StateService::update_last_opened(
        &app_handle,
        None,
...
pub async fn open_canvas(
    ...
    let canvas = CanvasService::open(path)
        .map_err(|e| e.to_string())?;
...
pub async fn list_canvases(vault_path: String) -> Result<Vec<CanvasInfo>, String> {
    VaultService::list_canvases(Path::new(&vault_path))
        .map_err(|e| e.to_string())
}
...
pub async fn rename_canvas(
    ...
    let canvas = CanvasService::rename(path, &new_name)
        .map_err(|e| e.to_string())?;
...
pub async fn delete_canvas(
    ...
    // Delete the canvas
    let canvas_id = CanvasService::delete(path)
        .map_err(|e| e.to_string())?;
...
pub async fn update_canvas_tags(
    ...
    let canvas = CanvasService::update_tags(path, tags)
        .map_err(|e| e.to_string())?;
...
pub async fn update_canvas_description(
    ...
    let canvas = CanvasService::update_description(path, &description)
        .map_err(|e| e.to_string())?;
...
pub async fn load_canvas_state(canvas_path: String) -> Result<CanvasUIState, String> {
    CanvasService::load_state(Path::new(&canvas_path))
        .map_err(|e| e.to_string())
}
...
pub async fn save_canvas_state(
    ...
    CanvasService::save_state(Path::new(&canvas_path), &state)
        .map_err(|e| e.to_string())
}
