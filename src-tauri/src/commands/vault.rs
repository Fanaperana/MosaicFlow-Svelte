...
use crate::services::{VaultService, HistoryService, StateService};
...
pub async fn open_vault(
    ...
    let vault = VaultService::open(vault_path)
        .map_err(|e| e.to_string())?;
...
pub async fn rename_vault(
    ...
    let vault = VaultService::rename(path, &new_name)
        .map_err(|e| e.to_string())?;
...
pub async fn update_vault_description(
    ...
    let vault = VaultService::update_description(path, &description)
        .map_err(|e| e.to_string())?;
...
pub async fn get_vault_info(path: String) -> Result<Option<VaultInfo>, String> {
    VaultService::get_info(Path::new(&path))
        .map_err(|e| e.to_string())
}
